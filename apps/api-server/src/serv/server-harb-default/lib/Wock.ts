...
class WockConnection {
	...
	logTrace: any;
	onceOff: boolean;
	timeoutPing: NodeJS.Timeout;
	timeoutWait: NodeJS.Timeout;
	timeoutCount: any;
	constructor(wock, wocker, $, maresClose) {
		...
		wock.on('message', async raw => {
			...
			const handle = this.wock.handles[event.type];
			if (handle) {
...
class Wocker {
	server: any;
	route: string;
	wockConnections: Set<unknown>;
	handles: { ping(wock: any): void; };
	...
	/** 批量发送事件 */
	cast(type, ...data) {
		this.wockConnections.forEach((wockConnection:any) => {
			try {
