...
const openTheStream = function(path, sizeFileLog, countBuckup, option) {
	return new streams.RollingFileStream(path, sizeFileLog, countBuckup, option)
		...
		.on('drain', () => process.emitWarning('log4js:pause', null));
};
...
const fileAppender = function(path, sizeFileLog, countBackup, option) {
	...
	const app = function(event) {
		...
		if(log !== undefined && !writer.write(log + (option.eol ?? EOL), 'utf8')) {
			process.emitWarning('log4js:pause', null);
		}
	};

	app.reopen = function() { writer.end(() => writer = openTheStream(path, sizeFileLog, countBackup, option)); };

	app.sighupHandler = function() { app.reopen(); };
